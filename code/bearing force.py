...
import bearing_vibration_combined_equaiton as vib
...
for k in range(len(t)):
    ...
    for j in range(n):
        ...
        heav = heavside(delta_value)
        if delta_value >= 0:
            fx_array[j] = heav * (Kb * (delta_value**e) * np.cos(fi[j]))
            fy_array[j] = heav * (Kb * (delta_value**e) * np.sin(fi[j]))
